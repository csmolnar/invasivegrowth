...
%add black borders if necessary, else crop image
if leftCrop==0
  ImBWc=[zeros(size(ImBWc,1),1),ImBWc];
  leftCropT=1;
else
  leftCropT=leftCrop;
end
topCrop = min(rows)-1;
if topCrop==0
  ImBWc=[zeros(1,size(ImBWc,2));ImBWc];
  topCropT=1;
else
  topCropT=topCrop;
end
rightCropT = max(columns)+1;
if rightCropT==size(image,2)+1;
  ImBWc=[ImBWc,zeros(size(ImBWc,1),1)];
  rightCropT=size(ImBWc,2);
end
bottomCropT = max(rows)+1;
if bottomCropT==size(image,1)+1;
  ImBWc=[ImBWc;zeros(1,size(ImBWc,2))];
  bottomCropT=size(ImBWc,1);
end
ImBWc=ImBWc(topCropT:bottomCropT,leftCropT:rightCropT);
